...
class NeuralNetwork:
    ...
    def back_error_prop(val):
        ...
        de_dt1 = NeuralNetwork.derivative_leaky_relu(NeuralNetwork.hidden) * de_dh1
        ...
        NeuralNetwork.T = diff @ np.transpose(NeuralNetwork.W_) @ NeuralNetwork.derivative_leaky_relu(
    ...
    def forward():
        ...
        NeuralNetwork.hidden = NeuralNetwork.leaky_relu(with_por)

        hid_w = NeuralNetwork.hidden @ NeuralNetwork.W_
        without_por = hid_w - NeuralNetwork.T_
        NeuralNetwork.output = NeuralNetwork.leaky_relu(without_por)
    ...
    @staticmethod
    def leaky_relu(vector, alpha=0.2):
    ...
    @staticmethod
    def derivative_leaky_relu(vector, alpha=0.2):
        for x in range(len(vector)):
            number = vector[x]
            if number > 0:
                vector[x] = 1.0
